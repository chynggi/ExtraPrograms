...
@author: vision
'''
from music21 import converter, instrument, note as note_, chord, midi, stream
import glob
import numpy as np
...
def MakeModuleandSave(path,modelpath,log):
    notes = []
    track = 0
    ...
    for i, file in enumerate(glob.glob(path+"/*.mid")):
        ...
        for element in notes_to_parse:
            if isinstance(element, note_.Note):
